...
#### fenutus
def draw_transform_current_property(lay, constraint, transformtype, propindex, overrideItem=None):
    ## TODO: subtract parent pone rot if local
        rw = lay.row(align=True)
        if overrideItem:
            tgt = overrideItem
        else:
            tgt = constraint.target
            if tgt.type == 'ARMATURE':
                subname = constraint.subtarget
                try:
                    tgt=tgt.pose.bones[subname]
                except:
                    pass
        splt = rw.split(factor = 0.4)
        colLabel = splt.column(align=True)
        colLabel.alignment = 'RIGHT'
        
        mtx = tgt.matrix_channel
        rotmode = tgt.rotation_mode
        consrotmode = getattr(constraint, "from_rotation_mode")
        
        
        rot = mtx.to_quaternion() if getattr(constraint, "from_rotation_mode") == 'Quaternion' else mtx.to_euler()
        if "LOCAL" in constraint.target_space and tgt.parent != None:
            rot = Vector(rot) - Vector(tgt.parent.matrix_channel.to_euler())
        
        prop = {'SCALE':tgt.matrix_channel.to_scale(),
        'LOCATION':tgt.matrix_channel.to_translation(),
        'ROTATION': rot
        }[transformtype]
        if overrideItem:
            colLabel.label(text = "Result:  ")
        else:
            colLabel.label(text = "Target's {0}:  ".format(transformtype[0]+transformtype[1:].lower()))
        val = prop[propindex + (consrotmode == 'QUATERNION')]
        txt = '{0:.3f} ({1:.2f}°)' if transformtype == 'ROTATION' else '{0:.4f}'
        splt.column(align=True).label(text = txt.format(val, val * 180 / 3.141592653))
#### fenutus
...
class ConstraintButtonsSubPanel:
    ...
    def draw_transform_from(self, context):
        ...
        #### fenutus
        draw_transform_current_property(col, con, con.map_from, 0)
        #### fenutus
        ...
        #### fenutus
        draw_transform_current_property(col, con, con.map_from, 1)
        #### fenutus
        ...
        #### fenutus
        draw_transform_current_property(col, con, con.map_from, 2)
        #### fenutus
    ...
    def draw_transform_to(self, context):
        ...
        actualme = None
        import bpy
        if con in list(bpy.context.object.constraints):
            actualme = bpy.context.object
        else:
            #if armature
            for bone in bpy.context.object.pose.bones:
                if len(bone.constraints)>0:
                    if con in list(bone.constraints):
                        actualme = bone
        #### fenutus
        ...
        #### fenutus
        draw_transform_current_property(col, con, con.map_to, 0, actualme)
        #### fenutus
        ...
        #### fenutus
        draw_transform_current_property(col, con, con.map_to, 1, actualme)
        #### fenutus
        ...
        #### fenutus
        draw_transform_current_property(col, con, con.map_to, 2, actualme)
        #### fenutus
